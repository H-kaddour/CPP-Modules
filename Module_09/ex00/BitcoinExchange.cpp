...
/*   Updated: 2023/03/18 21:47:40 by hkaddour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//std::pair<std::string, double> split(std::string data, char c)
...
void	BitcoinExchange::parse_input_file(std::string file)
{
	...
	for (int	i = 0; std::getline(input_file, data); i++)
	{
		...
		else
		{
			...
			{
				std::pair<std::string, std::string> sp = split(data, '|');

				//std::cout << sp.first << "|" << sp.second << std::endl;
				//trim_string(sp.first);
				std::cout << this->data.begin()->first << std::endl;
				//std::cout << std::find(&this->data.begin()->first, &this->data.end()->first, sp.first) << std::endl;;
				std::map<std::string, double>::iterator it;
				it = this->data.find("2009-01-02");
				std::cout << "l " << it->first << std::endl;
				if (it == this->data.end())
					std::cout << "7wi mok" << std::endl;
				std::cout << this->data.find("2009-01-02 ")->first << std::endl;
				exit(0);
				//if (!i)
				//{
...
BitcoinExchange::BitcoinExchange(void)
{
	...
	{
		add.first = split(data, ',').first;
		add.second = atof(split(data, ',').second.c_str());
		//this->data.insert(split(data, ','));
